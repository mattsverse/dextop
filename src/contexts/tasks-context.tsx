...
/**
 * Provides a React context that manages per-project task state, watches live task updates, and exposes helpers to initialize and dispose the task store.
 *
 * The provider keeps an in-memory map of tasks keyed by project path, synchronizes per-project watchers with the active project list, updates open-task counts, and supplies `getProjectTasks`, `initializeTasksStore`, and `disposeTasksStore` to descendants via context.
 *
 * @returns The context provider element that supplies the task store and control functions to its children
 */
